...
const registerAsAdmin = async (req, res) => {
    try {
        ...
        // Concatenate fullName and phoneNumber for classRollNumber
        // const classRollNumber = fullName.replace(/\s+/g, '').toUpperCase() + phoneNumber.replace(/\s+/g, '').toUpperCase();

        // Set password as last five digits of phoneNumber if not provided
        if (!password) {
...
const approveLORrequest = async (req, res) => {
    try {
        ...
        const imageBase64 = fs.readFileSync('./src/templates/bitLogo.png', 'base64');
        ...
        // Path where generated PDF will be Saved
         const pdfFileName = `${nextCount}_${user.classRollNumber.replace(/\//g, "")}.pdf`; // File name for the PDF 
        ...
        const mailOptions = {
            from: '"College ERP" <dummy2019@gmail.com>',
            to: user.email,
            ...
            <p style="font-size: 14px; margin: 0;">Designed and developed by Mr. Nitish Kumar <a href="https://www.linkedin.com/in/nitish-kumar-18bba4265/" target="_blank"><img src="https://upload.wikimedia.org/wikipedia/commons/c/ca/LinkedIn_logo_initials.png" alt="LinkedIn" style="width: 15px; vertical-align: middle;"></a></p>
            </div>
            </div>
            </div>
            `
...
const findLorsByclassRollNumber = async (req, res) => {
    try {
        const {classRollNumber } = req.params;

        // Find the user by classRollNumber
        const user = await User.findOne({ classRollNumber }).select(
            "-password -refreshToken"
        );
        ...
        return res.status(500).json({
            status: 500,
            success: false,
            message: "Internal Server Error on findLorsByclassRollNumber Controller",
            error: error.message
        });
    }
}
...
const findUserByclassRollNumber = async (req, res) => {
    const { classRollNumber } = req.params;
    ...
    try {
        ...
        return res.status(400).json({
            status: 400,
            success: false,
            message: "Insternal Server Error in finUserByclassRollNumber controller",
            error: error.message
        })
    }
}
...
const updateUserProfileByclassRollNumber = async (req, res) => {
    try {
        const { classRollNumber } = req.params;
        const { fullName, fatherName, programme, department, currentSemester, gender, email, phoneNumber } = req.body;

        const user = await User.findOne({ classRollNumber })
        if (!user) {
            return res.status(400).json({
                status: 400,
                success: false,
                message: "No user found from provided classRollNumber"
            });
        }
        ...
        const updatedUser = await User.findOneAndUpdate({ classRollNumber }, updateFields, { new: true });
        ...
        return res.status(500).json({
            status: 500,
            success: false,
            message: "Internal server error occurred in updateUserProfileBYclassRollNumber Controller",
            error: error.message
        });
    }
}
...
export { registerAsAdmin, updateLORrequest, approveLORrequest, rejectLORrequest, getAllPendingLOR, getAllApprovedLOR, getAllRejectedLOR, findLorsByclassRollNumber, findUserByclassRollNumber, updateUserProfileByclassRollNumber }