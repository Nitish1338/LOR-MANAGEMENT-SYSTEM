...
const registerUser = async (req, res) => {
    try {
        let { fullName, fatherName, classRollNumber, programme, department, password } = req.body;

        // Check if any required field is missing
        if (!fullName || !fatherName || !classRollNumber || !programme || !department || !password) {
...
const loginUser = async (req, res) => {
    try {
        let { classRollNumber, email, password } = req.body;

        // Convert RollNumber to uppercase 
        if (classRollNumber) {
            classRollNumber = classRollNumber.replace(/\s+/g, '').toUpperCase();
        }

        if (!((email && password) || (classRollNumber && password))) {
            return res.status(400).json({
                status: 400,
                success: false,
        ...
        if (classRollNumber) {
            user = await User.findOne({ classRollNumber });
        }
...
const forgetPassword = async (req, res) => {
    try {
        let { classRollNumber, email } = req.body;

        if (!classRollNumber || !email) {
            return res.status(400).json({
                status: 400,
                success: false,
                message: "All fields are required."
        ...
        // Remove all whitespace characters from classRollNumber fields
        classRollNumber = classRollNumber.replace(/\s+/g, '').toUpperCase();
